...
# Required first: stage the CellBender cell-barcode files into the WGS dir
# so the Singularity container can read them at /mnt/.  --bam-only/--demux-only
# preflight-check for these and error out if they are missing.
...
!!! warning "Run `--stage-barcodes` first"
    `--stage-barcodes` copies each library's `processed_feature_bc_matrix_cell_barcodes.csv` from the CellBender output into `${DEJAGER_WGS_DIR}` as `processed_feature_bc_matrix_cell_barcodes_{LibID}.csv`, the exact name the Singularity container expects at `/mnt/` (see [Singularity Bind Mounts](#singularity-bind-mounts)). The `--bam-only`, `--demux-only`, and `--all` modes run a preflight that **errors out** if these staged files are absent, so this step is not optional for the step-by-step path. `Demuxlet_DeJager.sh` (Option 1) performs the staging for you.
...
|------|-------|--------|------|-------|
| A. BAM Filter | 45 | 350 GB | 3 hours | Per library |
| B. Pileup + Demuxlet | 10 | 350 GB | 36 hours | Per library |
| C. Post-processing | 1 | 8 GB | 5 minutes | All libraries |
| Batch generator | 1 | 4 GB | < 1 hour | Once |

(Memory values are the `#SBATCH --mem=350G` allocations in the scripts generated by `Demuxlet_DeJager.py`.)

!!! warning "Memory Requirements"
    Both the BAM filtering (Step A) and pileup/demuxlet (Step B) steps request 350 GB of RAM, driven largely by the size of the ~84 GB SNP VCF. Monitor jobs and increase `--mem` if a large library is killed by the OOM handler.
